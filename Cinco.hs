-- Definir el tipo de dato para representar un piso
data Piso = Piso { año :: Int, metros :: Int, habitaciones :: Int, garaje :: Bool, zona :: Char, precio :: Float }
    deriving (Show)

-- Lista de pisos
pisos :: [Piso]
pisos = [Piso 2000 100 3 True 'A' 0,
...
añadirPrecio piso =
    let
        precioBase = (fromIntegral (metros piso) * 1000 + fromIntegral (habitaciones piso) * 5000 +
                      if garaje piso then 15000 else 0) * (1 - fromIntegral (2020 - año piso) / 100)
        precioConZona = if zona piso == 'B' then precioBase * 1.5 else precioBase
    in
        piso { precio = precioConZona }
...
main = do
    ...
    -- Imprimir el resultado de la función buscaPiso para la lista de pisos y el presupuesto dado
